...
學生已經穩固了基礎文法核心與進階句構，並具備多益實戰的除錯意識。目前已掌握以下【多益文法核心架構 V4.0】：
...
* 空間與時間概念 (In/On/At)：
    * In (大範圍/區間)：in 2026, in Taiwan. (例：The project launches *in* May.)
    * On (特定接觸面/特定日子)：on Monday, on the desk. (例：The meeting is *on* Friday.)
    * At (精確點/時刻)：at 10:30, at the station. (例：The service stops *at* midnight.)
...
## 【多益加分區：進階語法特例處理 (TOEIC Bonus: Advanced Exception Handling)】
此區塊為多益考試中的「高分攔截題」，不屬於常規主幹路徑，但遇到時需能快速調用以下特例協定：
* 假設倒裝 (Subjunctive Inversion)：卸載 If 標頭。
    * **例句：** *Had I known* the risk, I wouldn't have clicked it. (原：If I had known...)
    * **說明：** 省略 If 時，將 Had / Were / Should 提至句首擔任入口點。
* 否定倒裝 (Negative Inversion)：當否定副詞 (Never, Rarely, Hardly) 提至句首時，強制觸發「助動詞提升」。
    * **例句：** *Never have I seen* such a complex system. (原：I have never seen...)
    * **說明：** 否定詞提至句首產生強調效應，後方結構必須倒裝，將助動詞 (have) 移至主詞 (I) 前方。
* 強調句型 (Cleft Sentences)：使用 `It is/was ... that ...` 框架來高光標記特定變數。
    * **例句：** *It was* the network error *that* caused the delay.
    * **說明：** 將想強調的變數 (the network error) 放入框架中，其餘部分丟到 that 後面，達成聚焦效果。

---

## 補充協定：英文語句邏輯編譯 SOP（長難句與生字除錯）

### 核心思維
這是一套針對「長難句」與「生字恐懼症」的【英文語句邏輯編譯 SOP】。
核心理念：放棄「語感依賴與單字翻譯」，轉向「結構解耦與邏輯運算」。
當大腦遇到過載（腦袋一片空白）時，立刻停止「把選項塞進去唸唸看」的直覺操作，改為啟動系統化的除錯程序（Debugging Protocol）。

### 觸發條件 (Triggers)
當你在閱讀句子時遇到以下狀況，請立即啟動此 SOP：
* **Memory Overflow（記憶體溢出）：** 句子太長，找不到主詞與動詞在哪裡。
* **Undefined Variable（未定義變數）：** 出現不認識的單字，導致大腦當機、視覺停滯。
* **Plug-and-Play Failure（帶入法失效）：** 四個選項看起來都很順，或者都不順。

### 核心技術 (Core Techniques)
#### 1) 核心剝離術 (Core Stripping / Bracket Parsing)
* **原理：** 將「結構」與「語意」解耦 (Decoupling)。大腦無法同時處理複雜修飾與陌生語意。
* **動作：** 拿起筆（或在腦中），將所有「非核心雜訊」用 [中括號] 封裝並暫時隱藏。
* **攔截目標（雜訊）：**
  * 介系詞片語 (in, on, at, of, by, with, to...)
  * 關係子句 (who, which, that 引導的句子)
  * 修飾性副詞與形容詞
* **成效：** 剝離後，長難句會瞬間退化為國中程度的 `S + V + O` 或 `S + V + SC` 基本句型。

#### 2) 詞性預測協定 (Suffix-based POS Inference)
* **原理：** 英文是高度結構化的語言，單字的功能寫在「字尾（Suffix）」，而不是字義裡。
* **動作：** 遇到生字，絕對不要停下來猜中文意思。直接掃描字尾，貼上型別標籤 (Type Tag) 作為 Placeholder（佔位符）。
* **字尾判斷對照：**
  * `-tion, -ment, -ity, -ance, -ness` -> `[Noun]`（名詞變數）
  * `-ize, -ate, -en, -ify` -> `[Verb]`（動作函數）
  * `-ive, -able, -al, -ous, -ic` -> `[Adj]`（屬性修飾）
  * `-ly` -> `[Adv]`（環境參數）

#### 3) 正負電荷判定 (Charge Detection)
* **原理：** 遇到影響句意的關鍵生字，透過上下文推導其情緒極性。
* **動作：** 尋找句中已知的動詞或結果（如：praised 讚賞、failed 失敗），將生字標記為 `[+]` 正面參數或 `[-]` 負面參數，這足以應付 80% 的閱讀理解題。

### 標準作業程序 (Execution SOP)
面對任何多益文法題，強制執行以下迴圈：
* **步驟一：盲測屏蔽 (No-Option Rule)**
  * **指令：** 嚴禁先看 A, B, C, D 選項。先看選項會污染原始邏輯判斷。
* **步驟二：雜訊封裝與主幹定位 (Encapsulate & Locate)**
  * **指令：** 執行「核心剝離術」。畫出 [雜訊]，找出真正的「主詞 (Subject)」與「核心動詞缺口 (Verb Gap)」。
  * **檢核：** 確認主詞的型別（單數 Singular / 複數 Plural）。
* **步驟三：權限檢核 (Permission Gatekeeper)**
  * **指令：** 將剝離後的主詞與動詞進行邏輯連線。
  * **運算：** 主詞具備執行該動作的權限嗎？
    * 具備 (Active) -> 主動語態
    * 不具備 (Passive) -> 鎖定被動介面 `be + V.p.p.`
* **步驟四：環境變數掃描 (Environment Variable Scan)**
  * **指令：** 掃描全句，尋找「時間錨點」與「邏輯觸發詞」。
  * **目標：**
    * 時間指標：recently (現在完成)、by next week (未來完成)、last night (過去)
    * 強制指令：essential that...（強制還原原形）
* **步驟五：預測輸出 (Predictive Execution)**
  * **指令：** 在腦中生成虛擬答案。例如：「這個空格需要一個複數、過去式、被動語態的動詞」。
  * **核對：** 打開選項，尋找與你預測「型別完全對齊」的答案。

### 壓力測試指標 (Stress Test)
如何證明你掌握了這套思維？
當題目塞滿了像 `vlaritacious`（假形容詞）或 `flurm`（假名詞）這種毫無意義的偽造單字時，你依然能透過 `-ous` 字尾判定修飾關係，透過位置判定主詞，並精準選出正確的動詞型態。這代表你已經從「背單字機器」進化為「邏輯編譯器」。
